...
const ChatInterface = ({ selectedState }: ChatInterfaceProps) => {
  const { user } = useAuth();
          ...
          {messages.map((msg, i) => (
            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] px-5 py-4 rounded-2xl ${msg.role === 'user' ? 'bg-secondary text-secondary-foreground rounded-tr-none' : 'bg-muted text-primary rounded-tl-none'}`}>
                ...
                {msg.isCalculationResult && msg.calculatedAmount && (
                  <Button
                    onClick={() => handleProceedToPayment(msg.calculatedAmount!)}
                    className="mt-4 w-full"
                    size="sm"
                    disabled={isPaymentLoading}
                  >
                    {isPaymentLoading ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <CreditCard className="mr-2 h-4 w-4" />}
                    {user ? `Proceed with Payment (RM ${msg.calculatedAmount.toFixed(2)})` : 'Log in to Pay'}
                  </Button>
                )}
              </div>
            </div>
